  ...
  .controller("PainelCtrl", function ($scope, $http) {
    ...
    $scope.changeUrl = function () {
      ...
      if ($scope.config.url.substr(-1) === "/") {
        $scope.config.url = $scope.config.url.substr(
          0,
          $scope.config.url.length - 1
        );
      }
    ...
    $scope.chamar = function () {
      if (PainelWeb.started && $scope.senhas.length > 0) {
        ...
        if (
          $scope.ultima.texto !== "A000" &&
          $scope.ultima.texto !== senha.texto
        ) {
          // removendo duplicada
    ...
    $scope.run = function () {
      PainelWeb.started =
        $scope.config.unidade.id > 0 && $scope.config.servicos.length > 0;
      $.i18n.init(
        {
    ...
    function speechParams() {
      return {
        ...
        usarVozNavegador: $scope.config.usarVozNavegador,
        mode: $scope.config.modoVoz || 'extenso',
      };
    }
  })
  .filter("pad", function () {
...
var PainelWeb = {
  ...
  blink: function (elem) {
    ...
    setTimeout(function () {
      ...
      elem.css(
        "visibility",
        elem.css("visibility") === "visible" ? "hidden" : "visible"
      );
      if (count < 5) {
  ...
  Speech: {
    ...
    play: function (senha, params) {
      ...
      // Usar Web Speech API (voz do navegador)
      if (params.usarVozNavegador && SpeechService.isSuportado) {
        this.playing = true;
        SpeechService.falarSenha(senha, params, function() {
          PainelWeb.Speech.playing = false;
          PainelWeb.trigger("callend");
        });
        return;
      }

      // Fallback: arquivos MP3 estáticos
      // "senha"
      this.queue.push({ name: "senha", lang: params.lang });
      // sigla + numero
      var text = params.zeros
        ? $.painel().format(senha)
        : senha.sigla + senha.numero;
      for (var i = 0; i < text.length; i++) {
        this.queue.push({
          name: text.charAt(i).toLowerCase(),
          lang: params.lang,
        });
      }
      if (params.local) {
  ...
  Config: {
    load: function ($scope) {
      if (PainelWeb.Storage.get("theme")) {
        ...
        $scope.config.vocalizarLocal =
          PainelWeb.Storage.get("vocalizarLocal") === "1";
        $scope.config.usarVozNavegador =
          PainelWeb.Storage.get("usarVozNavegador") !== "0";
        $scope.config.modoVoz =
          PainelWeb.Storage.get("modoVoz") || "extenso";
        return true;
      }
      return false;
    },
    ...
    save: function ($scope) {
      ...
      PainelWeb.Storage.set(
        "vocalizarZero",
        $scope.config.vocalizarZero ? "1" : "0"
      );
      PainelWeb.Storage.set(
        "vocalizarLocal",
        $scope.config.vocalizarLocal ? "1" : "0"
      );
      PainelWeb.Storage.set("lang", $scope.config.lang);
      PainelWeb.Storage.set(
        "usarVozNavegador",
        $scope.config.usarVozNavegador ? "1" : "0"
      );
      PainelWeb.Storage.set("modoVoz", $scope.config.modoVoz || "extenso");
    },
  },
...
Array.prototype.contains = function (elem) {
  for (var i = 0; i < this.length; i++) {
    if (
      // se for senha
      (elem.sigla &&
        this[i].sigla === elem.sigla &&
        this[i].numero === elem.numero) ||
